      ...
      integer cudaStat4,cudaStat5,cudaStat6,cudaStat7
...
c     cudaMemcpyHostToDevice=1
      ...
      cudaStat7 = cuda_malloc(ynp1,       2*n*8) 
      if ((cudaStat1 /= 0) .OR. 
     $    (cudaStat2 /= 0) .OR. 
     $    (cudaStat3 /= 0) .OR. 
     $    (cudaStat4 /= 0) .OR. 
     ...
     $                                    nnz_vector*8,1)
      cudaStat7 = cuda_memcpy_fort2c_real(ynp1,      ynp1HostPtr,        
     $                                    2*n*8,1)
      if ((cudaStat1 /= 0) .OR. 
     $    (cudaStat2 /= 0) .OR. 
     $    (cudaStat3 /= 0) .OR. 
...
c     CUSPARSE_STATUS_SUCCESS=0  
c     CUSPARSE_INDEX_BASE_ONE=1
c      call get_shifted_address(y,n*8,ynp1)
c      status= cusparse_dsctr(handle, nnz_vector, xVal, xInd, 
c     $                       ynp1, 1)
c      if (status /= 0) then 
...
c         write(*,*) "Scatter from sparse to dense vector failed"
c         stop 1
c      endif  
...
c     check the results 
      ...
      if ((DABS(yHostPtr(1) - 10.0)    .GT. epsilon)  .OR. 
